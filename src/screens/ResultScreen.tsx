...
export default function ResultScreen() {
  ...
  const message = pct >= 90 ? 'מעולה!' : pct >= 70 ? 'כל הכבוד!' : pct >= 50 ? 'המשך להתאמן' : 'לא נורא, נסה שוב';
  ...
  return (
    <SafeAreaView style={s.safe}>
      <View style={s.container}>
        ...
        <Text style={s.pct}>{pct}% נכון</Text>
        ...
        <View style={s.infoBox}>
          <IR label="רמה"  value={`${levelId} — ${level.nameHe}`} />
          <IR label="תווים" value={String(total)} />
          <IR label="נכון"  value={String(correct)} />
          <IR label="שגוי"  value={String(total - correct)} />
        </View>

        <TouchableOpacity style={s.btn} onPress={() => navigation.navigate('Practice', { levelId, clef, noteCount, bothMode })}>
          <Text style={s.btnTxt}>נסה שוב — אותה רמה</Text>
        </TouchableOpacity>
        <TouchableOpacity style={s.btnSec} onPress={() => navigation.navigate('Home')}>
          <Text style={s.btnSecTxt}>חזרה לתפריט</Text>
        </TouchableOpacity>
      </View>
    </SafeAreaView>
  );
...
const s = StyleSheet.create({
  ...
  emoji: { fontSize: 72 },
  message: { fontSize: 28, fontWeight: '800', color: C.text, marginTop: 8 },
